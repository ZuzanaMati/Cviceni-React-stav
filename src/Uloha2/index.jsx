...
const Pocitadlo = () => {
	const [cislo, setCalculation] = useState(0)

	return (
		<>
			<h3>Počítadlo: {cislo}</h3>
			<button onClick={() => { setCalculation(cislo - 5) }}>-5</button>
			<button onClick={() => { setCalculation(cislo - 1) }}>-1</button>
			<button onClick={() => { setCalculation(0) }}>vynulovat</button>
			<button onClick={() => { setCalculation(cislo + 1) }}>+1</button>
			<button onClick={() => { setCalculation(cislo + 5) }}>+5</button>
		</>
	)
